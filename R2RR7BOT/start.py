...
from R2RR7BOT import __version__
...
@Client.on_message(
    command(["jhi88j", f"sthu8art@{BOT_USERNAME}"]) & filters.private & ~filters.edited
)
...
@Client.on_message(
    command(["big7yuy", f"alivhh77e@{BOT_USERNAME}"]) & filters.group & ~filters.edited
)
...
@Client.on_message(command(["pingu88u", f"pjiujing@{BOT_USERNAME}"]) & ~filters.edited)
        ...
        if member.id == bot_id:
            return await m.reply(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton("📣 قناة السورس", url=f"https://t.me/R2RR7"),
                            InlineKeyboardButton("💭 المبرمج", url=f"https://t.me/R2RR7")
                        ],
                        [
